...
namespace dotnet5781_00_2514_6512
{
    class TheBus :IComparable
    {
        ...
        public TheBus(List<BusLine> stations, BusStop firstStation, BusStop lastStation, int busLine, string area)
        {
            Stations = stations;
        ...
        {
            string str = $"Busline:{this.BusLine},Area:{this.Area},All_stations:";
            foreach (BusLine bs in Stations)
            {
                str += bs.ToString();
                str += " ";
            }
        ...
        public void AddOrRemove(BusLine action)
        {
                ...
                if (success)
                    switch (todo)
                    {
                        ...
                        case 2:
                            ...
                            {
                                int i=0;
                                foreach (BusLine it in Stations)
                                {
                                    if(it.getkey() == todo)
                                    {
        ...
        public bool exists(BusLine action)
        {
            foreach (BusLine b1 in Stations)
            {
        ...
        internal int TimeOrDistance(BusLine a, BusLine b, int action)
        {
        ...
        public int distance(BusLine a, BusLine b)
        {
            return TimeOrDistance(a, b, 1);
        }
        public int time(BusLine a, BusLine b)
        {
            return TimeOrDistance(a, b, 0);
        }

        public BusLine subRoute(BusLine a, BusLine b)
        {
            ...
            return new BusLine(sum_time, sum_distance, newnum, f1, f2, adress);
        }

        public int CompareTo(ref List<BusLine> other_route,BusLine a, BusLine b)
        {
