...
namespace DalObject
{
    sealed class DalObject : IDL // implamenting the interface
    {
        ...
        {
            Bus in_system = AllData.BusList.Find(bus2 => comparearr(bus2.licensePlateArray, bus.licensePlateArray)); // lambda with bus2 as the function
            if (in_system != null)
                throw new InsystemException("bus already in system");
            AllData.BusList.Add(bus.Clone()); // add the clone of the bus from the entry. not sure if this is needed however.
        }
        // cRud
        ...
        public void addStation(LineStation line)
        {
            ...
            AllData.LineStationList.Add(line.Clone());
        }
